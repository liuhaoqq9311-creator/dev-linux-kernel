 ...
 * lazily allocated and freed swap device info structs, and SWP_USED indicates
 * which devices are in use and which can be reused.
 *
...
/*
 * Mark a bad or masked-out slot during initialization. The cluster will stay
 * off the free list and its usage counter will be increased by 1.
 */
				       ...
				       unsigned long offset, bool mask)
{
	...
	if (offset >= si->max) {
		if (!mask)
			pr_debug("Ignoring bad slot %lu (max: %u)\n", offset, si->max);
	} else if (!mask) {
		/*
		 * Account it, but skip the header slot: si->pages starts as
		 * si->max - 1. Also skip the tail mask beyond si->max.
		 */
		if (offset)
			si->pages--;
		if (!si->pages) {
					 ...
					 unsigned int ci_off)
{
	...
	if (likely(folio)) {
		...
		/* Set a fake shadow as a placeholder. */
 ...
 * take its place: swap slots allocated to a folio start with count == 0 and
 * are pinned by swap cache. The swap cache pin doesn't increase the count.
 * This helper sets the initial count == 1 and then increases it as mappings
 * are replaced by swap entries.
 *
...
/*
 * Free swap slots after their count dropped to zero, or will become zero after
 * putting the final reference.
 */
				 ...
				 unsigned int ci_start, unsigned int nr_pages)
{
	...
	do {
		...
		/* Release the last ref, or the slot after swap cache is dropped. */
 ...
 *
 * Holding the folio lock ensures the folio won't go from not-swapped to
 * swapped because the initial count increment can only be done by
 * folio_dup_swap(), which also locks the folio. A concurrent count decrement
 * through swap_put_entries_direct() is still possible, so this may return a
 * false positive.
 *
 * Context: Caller must ensure the folio is locked and in the swap cache.
 */
				    ...
				    unsigned long maxpages)
{
	...
	/*
	 * Mark unusable pages as unavailable. This includes the header page,
	 * bad pages, and the EOF tail of the last cluster. The clusters are
	 * not free yet, so no list operations are involved here.
	 */
	...
	for (i = 0; i < swap_header->info.nr_badpages; i++) {
		...
		if (!page_nr || page_nr > swap_header->info.last_page) {
			pr_warn("Bad slot offset is out of border: %u (last_page: %u)\n",
				page_nr, swap_header->info.last_page);
			err = -EINVAL;
			goto err;
		}
		err = swap_cluster_setup_bad_slot(si, cluster_info, page_nr,
						 false);
		if (err)
			goto err;
	}
...
SYSCALL_DEFINE2(swapon, const char __user *, specialfile, int, swap_flags)
{
	...
	/*
	 * Allocate or reuse an existing !SWP_USED swap_info. The returned si
	 * stays inaccessible until enable_swap_info() resurrects its percpu
	 * ref and exposes it to the rest of the system.
	 */
	si = alloc_swap_info();
	if (IS_ERR(si))
		return PTR_ERR(si);
 ...
 *
 * Returns 0 for success, or -ENOMEM if the extend table is required but could
 * not be atomically allocated. Returns -EINVAL if the swap entry is invalid,
 * which might occur if a page table entry has got corrupted.
 *
 * Context: Caller must ensure there is no race condition on the reference
 * owner. e.g., locking the PTL of a PTE containing the entry being increased.
 * Also the swap entry must have a count >= 1. Otherwise folio_dup_swap() must
 * be used.
 */
int swap_dup_entry_direct(swp_entry_t entry)
{
	...
	/*
	 * Direct references such as PTEs may only bump entries that already
	 * carry a swap count.
	 */
