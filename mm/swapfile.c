...
new_cluster:
	...
	if (order < PMD_ORDER) {
		found = alloc_swap_scan_list(si, &si->nonfull_clusters[order], folio,
					     true);
		if (found)
			goto done;
	}
	...
	if (order < PMD_ORDER) {
		 ...
		 */
		found = alloc_swap_scan_list(si, &si->frag_clusters[order], folio,
					     false);
		if (found)
			goto done;
	}
 ...
 * Qualified value includes 1 or SWAP_HAS_CACHE.
 */
