...
/*
 * The cluster corresponding to page_nr will be used to store a bad
					    ...
					    struct folio *folio, unsigned long offset)
{
	...
	for (end -= nr_pages; offset <= end; offset += nr_pages) {
		...
		if (!__swap_cluster_alloc_entries(si, ci, folio,
				     ...
				     bool reclaim_cache)
{
	...
	do {
		...
		if (swp_tb_get_count(swp_tb) == 1) {
			...
			if (!swap_cache_has_folio(swp_entry(si->type, cur)))
				need_reclaim = true;
		}
		/*
		 * If count is 1 and swapcache is dropped, the slot could be freed
		 * after put. But the slot might still have a swap cache folio,
		 * and should be reclaimed before freeing. After reclaim, the
		 * slots will be freed when folio is removed from swap cache
		 * (__swap_cache_del_folio).
		 */
		__swap_cluster_put_entry(ci, ci_off);
		if (ci_batch != -1) {
			__swap_cluster_free_entries(si, ci, ci_batch,
						    ci_off - ci_batch);
			ci_batch = -1;
		}
next:
 ...
 *
 * In swap_dup_entries_cluster(), the swap table need to be checked before
 ...
 *
 * Returns 0 for success, or -ENOMEM if the swap count overflowed and
 * extended table allocation failed. Returns -EINVAL for invalid entries.
 *
 * Context: Caller must ensure there is no race condition on the reference
